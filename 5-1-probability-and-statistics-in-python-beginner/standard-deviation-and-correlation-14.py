...
for i in values:
    median_difference_sum += (i - values_median)
...
def std(column):
    sum_column = 0
    for i in column:
        sum_column += (i-(column.mean()))**2
    std_is = (sum_column/len(column))**(1/2)
    return std_is
...
probabilities2 = norm.pdf(np.arange(-10,10,0.1), 0, 2)
plt.plot(np.arange(-10,10,0.1), probabilities2)
plt.show()
