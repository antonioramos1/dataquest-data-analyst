...
min_high_pop = min(high_pop['median_income'])
lowest_income_high_pop_county = high_pop['county'][high_pop['median_income'] == min_high_pop]
...
random_sequence = [random.randint(0, 10) for i in range(10)]
...
random.seed(20)
new_sequence = [random.randint(0,10) for i in range(10)]
print(new_sequence)
...
random.seed(1)
mid_sample = [roll() for _ in range(100)]
plt.hist(mid_sample, 6)
plt.show()
...
random.seed(1)
plt.hist(probability_of_one(300, 10000), 20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()
...
deviations_from_mean = (.18 -large_sample_mean) /large_sample_std

over_18_count = len([sample for sample in large_sample if sample >= .18])
...
random_means = [select_random_sample(1000)['median_income'].mean() for i in range(100)]
plt.hist(random_means, 20)
plt.show()
...
random.seed(1)
random_samples = [select_random_sample(100) for i in range(1000)]
mean_ratios = [(i['median_income_hs'] / i['median_income_college']).mean() for i in random_samples]
...
significance_value = len([i for i in mean_ratios if i >= .675]) / len(mean_ratios)