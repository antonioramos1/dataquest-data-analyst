...
ordinal_scale={'none':0, 'some':0, 'a few':0, 'a lot':0}

for i in survey_responses:
    if i == 'none' and 'none' in ordinal_scale:
        ordinal_scale[i] =0
    elif i == 'a few'and 'a few' in ordinal_scale:
        ordinal_scale[i] +=1
    elif i =='some' and 'some' in ordinal_scale:
        ordinal_scale[i] +=2        
    elif i == 'a lot' and 'a lot' in ordinal_scale:
        ordinal_scale[i] +=3

average_smoking = sum(ordinal_scale.values())/len(survey_responses)
...
#[(a,b) for (a,b) in zip(gender, savings)]

male=0
female=0

for i in [(a,b) for (a,b) in zip(gender, savings)]:
    if i[0] == 'male':
        male += i[1]
    if i[0] == 'female':
        female += i[1]
female_savings = female/gender.count('female')
male_savings = male/gender.count('male')
...
plt.axvline(test_scores_negative.mean(), color="g")
...
plt.hist(test_scores_positive)
plt.axvline(numpy.mean(test_scores_positive))
plt.show()
